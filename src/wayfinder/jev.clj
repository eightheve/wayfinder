...
(defn evaluate
  ...
  (when (and (available? cfg) (seq questions))
    (let [api-key (api-key cfg)
      ...
      ;; Misbuilt states must fail fast, not burn API errors: Jev rejects
      ;; requests over ~32k tokens with a 400, and empirically ~32k tokens ≈
      ;; well under 100k chars, so anything past this bound is a caller bug.
      (if (> (count body) 100000)
        (do (println (format "[jev] request of %d chars exceeds the 100k safety budget — skipping" (count body)))
            nil)
        (loop [attempt 1]
          (let [resp (try
                       @(http/post url
            ...
            (cond
              ...
              :else
              (do
                (println (format "[jev] FAILED (status %s, attempt %d, %d chars): %s"
                           (:status resp) attempt (count body)
                           (trunc (or (:body resp)
